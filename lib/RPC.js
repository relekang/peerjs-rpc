...
RPC.prototype.invoke = function(id, func, args, callback) {
    var payload = {
        ...
        'args': args,
        'orig': this.id,
        'signature': this._createSignature(this.id, 'invoke')
    };
...
RPC.prototype.attr = function(id, attr, callback) {
    var payload = {
        ...
        'attr': attr,
        'orig': this.id,
        'signature': this._createSignature(this.id, 'attr')
    };
    return this._send(id, payload).nodeify(callback);
};
RPC.prototype.ping = function(id, callback) {
    var payload = {
        'type': 'rpc-ping',
        'orig': this.id,
        'signature': this._createSignature(this.id, 'ping')
    };
    return this._send(id, payload).nodeify(callback);
};
RPC.prototype._createSignature = function(id, func) {
    return sha1('' + Date.now() + ':' + id + ':' + func + '');
};
...
RPC.prototype._send = function(id, payload) {
    ...
    return this._getConnection(id).then(function(connection) {
        return new Promise(function(resolve) {
            ...
            __scope_21__._log('Sending payload: ', payload);
        });
    });
};
...
RPC.prototype._handlers = function() {
    var __scope_30__ = this;
    return {
        'rpc-ping': function(connection, data) {
            var payload = {
                'type': 'rpc-pong',
                'signature': data.signature,
                'orig': __scope_30__.id
            };
            __scope_30__._log('Sending payload: ', payload);
            connection.send(payload);
        },
        'rpc-attr': function(connection, data) {
            var payload = {
                'type': 'rpc-attr-return',
                'data': __scope_30__.scope[data.attr],
                'signature': data.signature,
                'orig': __scope_30__.id
            };
            __scope_30__._log('Sending payload: ', payload);
        ...
        'rpc-invoke': function(connection, data) {
            var __scope_29__ = this;
            if (data.func in __scope_30__.scope) {
                ...
                data.args.push(function(err, result) {
                    ...
                    __scope_29__._log('Sending payload: ', payload);
                    connection.send(payload);
                });
                __scope_30__.scope[data.func].apply(__scope_30__.scope, data.args);
            } else {
                var payload = {
                    'type': 'rpc-return',
