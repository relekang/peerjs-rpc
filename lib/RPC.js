...
RPC.prototype._send = function(id, payload) {
    var __scope_25__ = this;
    return this._getConnection(id).then(function(connection) {
        return new Promise(function(resolve, reject) {
            __scope_25__._timeouts[payload.signature] = setTimeout(function() {
                reject(new Error('Message timed out.'));
            }, __scope_25__.timeout);
            __scope_25__._callbacks[payload.signature] = function(result) {
                clearTimeout(__scope_25__._timeouts[payload.signature]);
                resolve(result);
            };
            connection.send(payload);
            __scope_25__._log('Sending payload: ', payload);
        });
    });
};
...
RPC.prototype._handlers = function() {
    var __scope_34__ = this;
    return {
        'rpc-ping': function(connection, data) {
            var payload = {
                'type': 'rpc-pong',
                'signature': data.signature,
                'orig': __scope_34__.id
            };
            __scope_34__._log('Sending payload: ', payload);
            connection.send(payload);
        },
        'rpc-attr': function(connection, data) {
            var payload = {
                'type': 'rpc-attr-return',
                'data': __scope_34__.scope[data.attr],
                'signature': data.signature,
                'orig': __scope_34__.id
            };
            __scope_34__._log('Sending payload: ', payload);
        ...
        'rpc-invoke': function(connection, data) {
            var __scope_33__ = this;
            if (data.func in __scope_34__.scope) {
                ...
                data.args.push(function(err, result) {
                    ...
                    __scope_33__._log('Sending payload: ', payload);
                    connection.send(payload);
                });
                __scope_34__.scope[data.func].apply(__scope_34__.scope, data.args);
            } else {
                var payload = {
                    'type': 'rpc-return',
